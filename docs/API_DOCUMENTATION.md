...
2. [Data models](#2-data-models)
   ...
   - [`AiBriefing` & `AiSuggestion`](#aibriefing--aisuggestion-libservicesai_servicedart)
...
4. [Services](#4-services)
   ...
   - [`AiService`](#aiservice-libservicesai_servicedart)
...
5. [Providers (state)](#5-providers-state)
   ...
   - [`FarmingProvider`](#farmingprovider-libprovidersfarming_providerdart)
...
### `AiBriefing` & `AiSuggestion`  *(`lib/services/ai_service.dart`)*
```dart
class AiBriefing {
...
Lightweight wrapper around Google Gemini for generating weather briefings and activity suggestions.

- `Future<AiBriefing?> generateBriefing(String cityName, CurrentWeather current, List<HourlyForecast> hourly)`  Calls the Gemini model with a compact JSON context. It caches responses keyed by city and current hour. Returns null on any error (no key, offline, quota, timeout).
- `void clearCache()`  Clears the in-memory cache (e.g. when the city changes).
- Internal: Uses `GenerativeModel` configured with `application/json` response MIME type and 45s timeout.
...
### `FarmingProvider`
**File:** `lib/providers/farming_provider.dart`

State container for the Farmer's Corner screen.

```dart
class FarmingProvider extends ChangeNotifier {
  List<FarmAdvice> advisories;
  int overallScore;
  BdSeason season;
  bool isReady;
...
```
- Listens to `WeatherProvider` updates to recompute agricultural suitability metrics via `FarmingAdvisory.compute`.
